...
class VPGBuffer:
    ...
    def __init__(self, obs_dim, act_dim, size, gamma, lam):
        ...
        self.gamma = gamma
        self.lam = lam
        self.ptr, self.path_start_idx, self.max_size = 0, 0, size # self.max_size = number of steps per epoch
    ...
    def end_traj(self, last_val=0):
        ...
        self.ret_buf[path_slice] = discount_cumsum(self.rew_buf[path_slice], self.gamma) * np.power(self.gamma * np.ones(self.ptr-self.path_start_idx),np.arange(0, self.ptr - self.path_start_idx))
...
class Agent:
    ...
    def pi_update(self, data):
        ...
        #     loss -= ret[i] * self.ac.pi(obs[i], act[i])[1]

        # loss.backward()
        # self.pi_optimizer.step()

        # Advantage function (generalized advantage estimator):
        loss = torch.tensor(0, dtype = torch.float32)
        for i in range(len(obs)):
            loss -= phi[i] * self.ac.pi(obs[i], act[i])[1]
    ...
    def train(self):
        ...
        for epoch in range(epochs):
            ep_returns = [] # cumulative return (without discounting) of every episode in one epoch
